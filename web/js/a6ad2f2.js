...
Style_maille2.addRules([rule_obs_1, rule_obs_2, rule_obs_3, rule_cluster]);
...
function partialTaxonList(){
    $("#TaxonSelect").select2({
        ...
        initSelection: function(element, callback) {
            if(SelectedTaxon){
                $.ajax("query/taxons?id="+SelectedTaxon, {
                    dataType: "json"
                }).done(function(data) {
...
$(document).ready(function() {
    ...
    $('#FullTaxonListButton').click(function(){
        if(partialQueryEnabled){
            ...
            $("#TaxonSelect").replaceWith("<select data-placeholder='Choisir un taxon' id='TaxonSelect' class='LeftSelectInput' value=' '><option></option></select>");
            $("#TaxonSelect").select2();
            taxonsListUpdate();
            ...
            $("#TaxonSelect").select2("destroy");
            $("#TaxonSelect").replaceWith("<input data-placeholder='Choisir un taxon' type=hidden id='TaxonSelect' class='LeftSelectInput' value=' '/>");
...
function onFeatureSelect(feature){
    if(feature.cluster){
        ...
        popupContent = popupContent + "<b>Nom(s) du(des) taxon(s): </b><br/>" + feature.attributes.noms_taxon + "<br/>";
...
© IGN 2013, BD CARTO - © SANDRE 2013, SIE - © Muséum national d\'Histoire naturelle 2013, Espaces protégés - © GEOSIGNAL 2013, Carte routière'};//COPYRIGHT attribution
...
//Getting the data for the map features
        ...
        //Load Taxon Image
        var photoUrl = "query/photo?cd_ref=" +taxon;
        $.ajax({
            url: photoUrl,
            type     : "GET",
...
function ChangeRendu() {
    ...
    switch ($('input[name=RenduOptions]:checked', '#RenduSelectionForm').attr('value')) {
        ...
        case '2':
            ...
            $('#legendRendu').append("<div id='renduLegend'><div class='renduLegendSquare' style='background:#F6E497; border-color:#F6E497;'></div>Moins de 10 observations<div class='clearfix'></div><div id='renduLegend'><div class='renduLegendSquare' style='background:#C79F4B; border-color:#C79F4B'></div>Entre 10 et 100 observations<div class='clearfix'></div><div id='renduLegend'><div class='renduLegendSquare' style='background:#2E1C0B; border-color:#2E1C0B'></div>Plus de 100 observations");
            break;
        case '3':
